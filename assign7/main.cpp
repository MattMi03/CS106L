...
unique_ptr<ListNode<T>> create_list(const std::vector<T> &values)
{
  ...
  // 如果输入的 vector 为空，返回 nullptr
  if (values.empty())
    return nullptr;

  auto current = make_unique<ListNode<T>>(values[values.size() - 1]);

  for (int i = values.size() - 2; i >= 0; i--)
  {
    auto next_node = make_unique<ListNode<T>>(values[i]);
    next_node->next = std::move(current);
